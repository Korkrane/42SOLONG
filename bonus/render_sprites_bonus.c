...
/*   Updated: 2021/06/11 16:47:28 by bahaas           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	sprt_color(t_cub *cub, t_pos text, t_pos pos, int i)
{
	...
	color = grep_color(cub->text[2], text.x, text.y);
	background = grep_color(cub->text[2], 0, 0);
	//printf("color : %d && background : %d\n", color, background);
	if (color != background)
	{
		my_mlx_pixel_put(&cub->win, cub->sprt[i].pos_x + pos.x, pos.y, GREEN);
	}
}
...
void	sprt_display(t_cub *cub, int i)
{
	...
	{
			text.x = pos.x * cub->text[2].wid / cub->sprt[i].hei;
			pos.y = cub->sprt[i].top_px - 1;
			while (++pos.y < cub->sprt[i].bot_px)
			{
				text.y = (pos.y + (cub->sprt[i].hei / 2) -
					(cub->win.hei / 2)) * (cub->text[2].hei / cub->sprt[i].hei);
				if (text.y < 0)
					text.y = 0;
				/*
				printf("test text_pos.x : %f\n", text.x);
				printf("test text_pos.y : %f\n", text.y);
				printf("test pos.x : %f\n", pos.x);
				printf("test pos.y : %f\n", pos.y);
				printf("test i : %d\n", i);
	*/
...
void	fill_sprt_data(t_cub *cub)
{
	...
	{
		while(j < cub->data.rows)
		{
			k = 0;
			while(k < cub->data.cols)
			{
				if(cub->grid[j][k] == 'E')
				{
					top_px = cub->win.hei / cub->data.rows * j;
					bot_px = cub->win.hei / cub->data.rows * (j + 1);
					cub->sprt[16].hei = bot_px - top_px;
					cub->sprt[16].top_px = top_px;
					cub->sprt[16].bot_px = bot_px;
					cub->sprt[16].pos_x = 1.5;
					//printf("top px : %d\n", top_px);
					//printf("bot px : %d\n", bot_px);
					//printf("Exit pos x: %f\n", cub->sprt[16].pos.x);
					//printf("Exit pos y: %f\n", cub->sprt[16].pos.y);
